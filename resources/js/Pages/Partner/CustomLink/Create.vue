...
const FormSubmit = () => {
    form.post(route('partner.links.store'))
}
</script>

<template>
    <Head title="Tambah Custom Link Resmi" />

    <PartnerLayout>
        <div class="card-dashboard">
            <h2 class="card-title-dashboard">Tambah Custom Link Resmi</h2>
            <form class="form-dashboard" @submit.prevent="FormSubmit">
                    ...
                    <div class="flex-1">
                        <div class="mb-5">
                            ...
                            <select v-model="form.name" id="name" class="form-input-dashboard">
                                <option value="" disabled>Pilih jenis sosial media</option>
                ...
                <FormButton :href="route('partner.links.index')" text="Simpan" />
            </form>
        </div>
    </PartnerLayout>
</template>
